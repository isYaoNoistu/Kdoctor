...
func inferRootCauses(index map[string]model.CheckResult) []correlatedCause {
	...
	if isProblem(kraftController) || isProblem(kraftQuorum) {
		causes = append(causes, correlatedCause{
			Priority:   90,
			Confidence: "中高置信度主因",
			Summary:    "KRaft controller 或 quorum 存在异常，可能导致 metadata 不稳定、分区 leader 漂移或内部主题问题。",
			Actions:    mergeActionsWithFallback([]model.CheckResult{kraftController, kraftQuorum}, "优先确认 controller listener、quorum 多数派以及 controller 选举是否稳定。"),
		})
	}
	...
	if isProblem(topicLeader) || isProblem(topicReplica) || isProblem(topicISR) {
		limitations := []string{}
		...
		causes = append(causes, correlatedCause{
			...
			Actions:     mergeActionsWithFallback([]model.CheckResult{topicLeader, topicReplica, topicISR}, "检查受影响分区的 leader、ISR、磁盘与 broker 副本状态。"),
	...
	if isProblem(logs) {
		causes = append(causes, correlatedCause{
			...
			Actions:    firstActions(logs, "结合命中的日志指纹和解释结果，优先沿对应故障链路继续深挖。"),
			Limitations: []string{
